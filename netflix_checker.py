...
premium_users = set()  # {user_id_str} — users with premium check access
    ...
    try:
        if os.path.exists(PREMIUM_FILE):
            with open(PREMIUM_FILE) as f:
                premium_users = {l.strip() for l in f if l.strip()}
    except Exception:
        pass
...
def is_premium(uid: int) -> bool:
    """Admin is always premium. Others need explicit grant."""
    if uid in ADMIN_IDS: return True
    return str(uid) in premium_users
...
def gen_ios_nftoken(nid: str):
    ...
    try:
        r = requests.get(IOS_API_URL, params=IOS_PARAMS, headers=headers, timeout=12, verify=False)
...
@app.route('/')
def home():
    return "Netflix Bot Running 24/7 🎬", 200
...
def _self_ping():
    """Ping self every 14 min to prevent Render free tier spin-down."""
    import time as _t
    _t.sleep(60)  # wait 1 min for server to start
    url = os.environ.get("RENDER_EXTERNAL_URL", "")
    if not url:
        print("[Ping] RENDER_EXTERNAL_URL not set, skip self-ping")
        return
    while True:
        try:
            requests.get(f"{url}/ping", timeout=10)
            print(f"[Ping] Self-ping OK")
        except Exception as e:
            print(f"[Ping] Failed: {e}")
        _t.sleep(840)  # 14 minutes
...
def main():
    ...
    def group_tv_login(message):
        ...
        if len(parts) < 2 or not parts[1].strip():
            return bot.reply_to(
                ...
                "2️⃣ Select *Sign In → Sign in with code*\n"
                "3️⃣ Note the *8-digit code* shown\n"
                "4️⃣ Send: `/login YOUR_CODE`\n\n"
                "*Examples:*\n"
                "• `/login 12345678`\n"
                "• `/login 1234-5678`\n"
                "• `/login 1234 5678`\n\n"
                "⚡ Bot will auto-login from database",
                parse_mode='Markdown'
            )
        ...
        def _do_group_login():
            ...
            if ok:
                ...
                reason = _clean_api_msg(msg) if msg else "TV code expired — get a fresh code from your TV."
                # Also try to get a fresh TV link to show user
                tv_link_text = ""
                if valid_link3 and str(valid_link3).startswith("http"):
                ...
                result_text = (
                    ...
                    f"📺 *Code:* `{code_clean}`\n"
                    f"🕐 *Time:* {ts}\n\n"
                    f"⚠️ *Reason:* {reason}{tv_link_text}\n\n"
    ...
    def cmd_premium(message):
        """Admin: grant/revoke premium. Users: check status."""
        uid_self = message.from_user.id if message.from_user else message.chat.id
        parts = (message.text or "").split()
        if is_admin(uid_self) and len(parts) >= 3:
            action = parts[1].lower()
            try:
                target = int(parts[2])
            except ValueError:
                return bot.reply_to(message, "❌ Usage: `/premium grant 123456`\nor `/premium revoke 123456`", parse_mode='Markdown')
            if action in ("grant", "add", "give"):
                premium_users.add(str(target))
                _save_premium()
                try: bot.send_message(target, "💎 *Premium Unlocked!*\nYou now have access to full cookie details and premium features!", parse_mode='Markdown')
                except Exception: pass
                bot.reply_to(message, f"✅ Premium granted to `{target}`", parse_mode='Markdown')
            elif action in ("revoke", "remove", "del"):
                premium_users.discard(str(target))
                _save_premium()
                try: bot.send_message(target, "ℹ️ Your premium access has been revoked.", parse_mode='Markdown')
                except Exception: pass
                bot.reply_to(message, f"✅ Premium revoked from `{target}`", parse_mode='Markdown')
            elif action == "list":
                plist = "\n".join(premium_users) if premium_users else "None"
                bot.reply_to(message, f"💎 *Premium Users:*\n`{plist}`", parse_mode='Markdown')
            else:
                bot.reply_to(message,
                    "📋 *Premium Commands (Admin):*\n"
                    "`/premium grant USER_ID`\n"
                    "`/premium revoke USER_ID`\n"
                    "`/premium list`\n\n"
                    "Users send `/premium` to check their status.",
                    parse_mode='Markdown')
        else:
            status = "💎 *Premium*" if is_premium(uid_self) else "🔓 *Standard*"
            bot.reply_to(message, f"Your account: {status}\n\nPremium users get full cookie details.", parse_mode='Markdown')
    ...
    def cookie_check_start(message):
        ...
        if mode == "premium" and not is_premium(message.chat.id):
            bot.reply_to(message,
                "💎 *Premium Check*\n\n"
                "This feature requires *Premium access*.\n"
                "Contact admin to get premium.\n\n"
                "Use 🔍 *Free Check* for basic checking.",
                parse_mode='Markdown')
            return
    ...
    def handle_input(message):
        ...
        if not mode:
            # Check if it looks like a cookie (contains NetflixId or is a file)
            is_cookie_msg = False
            if message.content_type == 'document':
                is_cookie_msg = True
            elif message.text:
                txt = message.text.strip()
                is_cookie_msg = ('NetflixId' in txt or 'netflix.com' in txt.lower() or
                                 txt.startswith('[') or txt.startswith('{') or
                                 (len(txt) > 20 and ' ' not in txt[:50]))
            if is_cookie_msg:
                GLOBAL_EXECUTOR.submit(_handle_cookie_check, message, "premium")
                return
            return bot.reply_to(message, "❌ **Select a mode first!**", parse_mode='Markdown')
        if mode.get('stop'):
    ...
    def send_hit(chat_id, res, cookie, duration="N/A", include_screenshot=True, add_back_button=False):
        ...
        # Check if user is premium (full details) or normal (limited)
        _user_is_premium = is_premium(chat_id)

        if _user_is_premium:
            # Full details for premium/admin
            msg = (
                f"{th['header']}\n\n"
                ...
                f"{th['link_h']}\n"
                + (f"<b>├ 💻 PC:</b> <a href='{res['link']}'>🔗 Open PC</a>\n" if res.get('link') and str(res.get('link','')).startswith('http') else "")
                + (f"<b>├ 📱 Mobile:</b> <a href='{res['link2']}'>🔗 Open Mobile</a>\n" if res.get('link2') and str(res.get('link2','')).startswith('http') else "")
                + (f"<b>└ 📺 TV Login:</b> <a href='{res['link3']}'>🔗 Open TV</a>\n" if res.get('link3') and str(res.get('link3','')).startswith('http') else "<b>└ 📺 TV Login:</b> N/A\n")
                + f"<i>{th['valid']}</i>\n\n"
            ...
            # Limited details for normal users (API fields only)
            msg = (
                f"{th['header']}\n\n"
                f"<b>{th['status']}:</b> ✅ Active\n"
                f"<b>{th['region']}:</b> {esc(country_code)} {flag}\n\n"
                ...
                f"<b>🗓 Expiry:</b> {esc(data.get('expiry', 'N/A'))}\n\n"
                ...
                f"<b>👨‍💻 Admin:</b> <a href='https://t.me/F88UF'>Message Me</a>\n"
                f"<b>📢 Channel:</b> <a href='https://t.me/F88UF9844'>Join Channel</a>\n\n"
                f"ℹ️ <i>Full details available for Premium users</i>\n\n"
                f"<b>🍪 COOKIE COPY:</b>\n<code>{cookie}</code>"
            )
        ...
        # Screenshot only for premium users
        _show_screenshot = include_screenshot and res.get('screenshot') and is_premium(chat_id)
        if _show_screenshot:
            try:
...
if __name__ == "__main__":
    ...
    # Bot runs in background thread; Flask holds the main thread for Render
    _bt = _mt.Thread(target=main, daemon=True)
    _bt.start()
    port = int(os.environ.get("PORT", 10000))
