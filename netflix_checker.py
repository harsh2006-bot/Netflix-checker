...
def handle_input(message):
    ...
    if not mode: return bot.reply_to(message, "❌ **Select a Mode first!**")
    ...
    def background_task():
        ...
        for i, c in enumerate(cookies, 1):
            ...
            prog = int((i/total)*10)
            bar = "■"*prog + "□"*(10-prog)
            try: bot.edit_message_text(f"🚀 **Checking:** [{bar}] {int((i/total)*100)}%\nChecked: {i}/{total} | Hits: {hits_count}", uid, status_msg.message_id)
            except: pass
            res = check_cookie(c)
            if res.get("valid"):
                hits_count += 1
        ...
        bot.send_message(uid, f"✅ **Check Complete!** Found {hits_count} Hits.")
...
def send_hit(target, res, cookie):
    ...
    lines = [f"🌟 **NETFLIX PREMIUM ULTRA HIT** 🌟", "", f"🟢 **STATUS:** Active ✅"]
    ...
    lines.append(f"📋 **COOKIE (TAP TO COPY):**", f"<code>{esc(cookie)}</code>")
    lines.append("\n━━━━━━━━━━━━━━━━━━━━━━", f"👨‍💻 **Admin:** @F88UF | 📢 **Channel:** @F88UF9844")
    msg = "\n".join(lines)
    if res.get('screenshot'): bot.send_photo(target, io.BytesIO(res['screenshot']), caption=msg, parse_mode='HTML')
